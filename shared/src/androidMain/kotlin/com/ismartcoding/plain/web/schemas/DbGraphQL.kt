...
fun SchemaBuilder.addDbSchema() {
    ...
    query("dbTableRowCount") {
        resolver("table") { table: String -> dbTableRowCount(table) }
    }

    query("dbTableRows") {
        resolver("table", "offset", "limit") { table: String, offset: Int, limit: Int -> dbTableRows(table, offset, limit) }
    }

    query("dbTableInfo") {
        resolver("table") { table: String -> DbTableInfo(idKey = primaryKeyColumn(table)) }
    }
