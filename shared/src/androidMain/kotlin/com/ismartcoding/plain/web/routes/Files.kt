    ...
    get("/fs") {
        ...
        try {
            ...
            if (path.startsWith("content://")) {
                ...
                if (q["dl"] == "1") {
                    val fileName = (jsonName.ifEmpty { uri.lastPathSegment.orEmpty() }).urlEncode()
                ...
                call.response.header("Access-Control-Expose-Headers", "Content-Disposition")
                val fileName = (jsonName.ifEmpty { file.name }).urlEncode()
                if (q["dl"] == "1") {
                    call.response.header(
                        "Content-Disposition",
                        "attachment; filename=\"${fileName}\"; filename*=utf-8''${fileName}"
                ...
                if (w != null && h != null) {
                    val bytes = withIO { ThumbnailProvider.instance?.toThumbBytesAsync(appContext, file, w, h, centerCrop, mediaId, fileName) }
                    if (bytes != null) {
