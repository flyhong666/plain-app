...
import com.ismartcoding.plain.lib.apk.parser.StringPoolEntry
import com.ismartcoding.plain.lib.apk.struct.ResValue
import com.ismartcoding.plain.lib.apk.struct.ResourceValue
import com.ismartcoding.plain.lib.apk.struct.StringPool
import com.ismartcoding.plain.lib.apk.struct.StringPoolHeader
...
object ParseUtils {
    ...
    fun readStringPool(buffer: ByteBuffer, stringPoolHeader: StringPoolHeader): StringPool {
        ...
        val sorted =
            (stringPoolHeader.flags and StringPoolHeader.Companion.SORTED_FLAG.toLong()).toInt() != 0
        // string use utf-8 format if true, otherwise utf-16
        val utf8 = (stringPoolHeader.flags and StringPoolHeader.Companion.UTF8_FLAG.toLong()).toInt() != 0
...
//        ResValue resValue = new ResValue();
        ...
        return when (val dataType = Buffers.readUByte(buffer)) {
            ResValue.ResType.INT_DEC -> ResourceValue.Companion.decimal(
                buffer.int
            )

            ResValue.ResType.INT_HEX -> ResourceValue.Companion.hexadecimal(
                buffer.int
            )
            ...
            ResValue.ResType.REFERENCE ->
                try {
                    ResourceValue.Companion.reference(
                        buffer.int
                    )
                } catch (ex: Exception) {
                    null
            ...
            ResValue.ResType.INT_BOOLEAN -> ResourceValue.Companion.bool(
                buffer.int
            )
            ...
            ResValue.ResType.DIMENSION -> ResourceValue.Companion.dimension(
                buffer.int
            )

            ResValue.ResType.FRACTION -> ResourceValue.Companion.fraction(
                buffer.int
            )
