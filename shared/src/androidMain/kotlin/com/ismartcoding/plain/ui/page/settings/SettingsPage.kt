...
fun SettingsPage(navController: NavHostController, updateViewModel: UpdateViewModel, peerVM: PeerViewModel) {
    ...
    val awareSupported = WifiAwareTransport.isSupported()
    val awareChars = remember {
        if (isTPlus()) runCatching { wifiAwareManager.getCharacteristics() }.getOrNull() else null
    }
                ...
                item {
                    ...
                    PCard {
                        PListItem(
                            ...
                            value = com.ismartcoding.plain.getAndroidVersion(),
                        )
                        PListItem(
                        ...
                        if (AppFeatureType.CHECK_UPDATES.has()) {
                            PListItem(title = stringResource(Res.string.app_version), subtitle = com.ismartcoding.plain.getAppVersion(), action = {
                            ...
                            PListItem(title = stringResource(Res.string.app_version), value = com.ismartcoding.plain.getAppVersion())
                        }
                    }
                }
                item {
                ...
                if (developerMode) {
                    item {
                        ...
                        DeveloperSettingsCard(
                            ...
                            awareDataInterfaces = awareChars?.getNumberOfSupportedDataInterfaces(),
                            awareDataPaths = awareChars?.getNumberOfSupportedDataPaths(),
                        )
                    }
                }
