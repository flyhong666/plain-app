                ...
                LazyColumnScrollbar(state = scrollState) {
                    LazyColumn(
                        ...
                        items(items, key = { it.id }) { m ->
                            PackageListItem(item = m, iconProvider = { packageManager.getApplicationIcon(it) }, modifier = PlainTheme.getCardModifier(), onClick = { navController.navigate(Routing.AppDetails(m.id)) })
