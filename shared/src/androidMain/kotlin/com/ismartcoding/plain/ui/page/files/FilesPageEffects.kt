...
internal fun FilesPageEffects(
    ...
    BackHandler(enabled = previewerState.visible || filesVM.selectMode.value || filesVM.showSearchBar.value || filesVM.showPasteBar.value || filesVM.canNavigateBack()) {
        when {
            ...
            filesVM.showSearchBar.value -> {
                ...
                scope.launch(Dispatchers.Default) { filesVM.loadAsync(context) }
            }
    ...
    LaunchedEffect(Channel.sharedFlow) {
        Channel.sharedFlow.collect { event ->
            when (event) {
                is PermissionsResultEvent -> scope.launch(Dispatchers.Default) { filesVM.loadAsync(context) }
                ...
                is ActionEvent -> if (event.source == ActionSourceType.FILE) scope.launch(Dispatchers.Default) { filesVM.loadAsync(context) }
            }
        }
    }
}
