    ...
    LaunchedEffect(Channel.sharedFlow) {
        Channel.sharedFlow.collect { event ->
            if (event is PickFileResultEvent && event.tag == PickFileTag.POMODORO && event.uris.isNotEmpty()) {
                scope.launch {
                    try {
                        val file = context.contentResolver.queryOpenableFile(Uri.parse(event.uris.first()))
                        if (file != null) {
                            ...
                            FileHelper.copyFile(context, Uri.parse(event.uris.first()), dstFile.absolutePath)
