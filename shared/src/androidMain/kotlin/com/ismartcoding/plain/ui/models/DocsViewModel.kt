...
class DocsViewModel : BaseMediaViewModel<DDoc>() {
    ...
    override suspend fun loadAsync(tagsVM: TagsViewModel) = withIO {
        ...
        if (!trash.value) {
            val extGroups = DocMediaStoreHelper.getDocExtGroupsAsync(appContext, super.getQuery())
    ...
    fun delete(tagsVM: TagsViewModel, ids: Set<String>) {
        launchSafe {
            ...
            DocMediaStoreHelper.deleteRecordsAndFilesByIdsAsync(appContext, ids, trash.value)
            loadAsync(tagsVM)
