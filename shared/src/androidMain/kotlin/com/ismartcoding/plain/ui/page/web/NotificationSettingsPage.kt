...
fun NotificationSettingsPage(
    ...
    LaunchedEffect(Unit) {
        scope.launch(Dispatchers.Default) { vm.loadDataAsync(context) }
    }
                    ...
                    if (selectedAppsState.isNotEmpty()) {
                        ActionButtonMoreWithMenu { dismiss ->
                            PDropdownMenuItem(
                                ...
                                onClick = { dismiss(); scope.launch(Dispatchers.Default) { vm.clearAllAsync(context) } },
        ...
        LazyColumn(modifier = Modifier.padding(top = paddingValues.calculateTopPadding())) {
            item {
                ...
                NotificationFilterModeCard(
                    ...
                    onToggleMode = { scope.launch(Dispatchers.Default) { vm.toggleModeAsync(context) } }
                )
            ...
            if (!vm.isLoading.value) {
                items(selectedAppsState, key = { it.id }) { app ->
                    NotificationAppListItem(app = app, onRemove = {
                        scope.launch(Dispatchers.Default) { vm.removeAppAsync(context, app.id) }
                    })
                }
                item {
                    Button(
    ...
    if (vm.showAppSelector.value) {
        AppSelectorBottomSheet(
            ...
            onAppsSelected = { packageNames ->
                scope.launch(Dispatchers.Default) { vm.addAppsAsync(context, packageNames) }
