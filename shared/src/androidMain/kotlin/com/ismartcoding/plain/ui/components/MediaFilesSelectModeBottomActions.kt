            ...
            if (AppFeatureType.MEDIA_TRASH.has()) {
                if (isInTrashMode) {
                    IconTextSmallButtonRestore {
                        ...
                        when (vm) {
                            is ImagesViewModel -> vm.restore(tagsVM, dragSelectState.selectedIds.toSet())
                            is VideosViewModel -> vm.restore(tagsVM, dragSelectState.selectedIds.toSet())
                            is AudioViewModel -> vm.restore(tagsVM, dragSelectState.selectedIds.toSet())
                        }
                        dragSelectState.exitSelectMode()
                    }
                    IconTextSmallButtonDelete {
                        DialogHelper.confirmToDelete {
                            @Suppress("UNCHECKED_CAST")
                            when (vm) {
                                is ImagesViewModel -> vm.delete(tagsVM, dragSelectState.selectedIds.toSet())
                                is VideosViewModel -> vm.delete(tagsVM, dragSelectState.selectedIds.toSet())
                                is AudioViewModel -> vm.delete(tagsVM, dragSelectState.selectedIds.toSet())
                            }
                            dragSelectState.exitSelectMode()
                        }
                    }
                    ...
                    IconTextSmallButtonTrash {
                        @Suppress("UNCHECKED_CAST")
                        when (vm) {
                            is ImagesViewModel -> vm.trash(tagsVM, dragSelectState.selectedIds.toSet())
                            is VideosViewModel -> vm.trash(tagsVM, dragSelectState.selectedIds.toSet())
                            is AudioViewModel -> vm.trash(tagsVM, dragSelectState.selectedIds.toSet())
                        }
                        dragSelectState.exitSelectMode()
                    }
                }
