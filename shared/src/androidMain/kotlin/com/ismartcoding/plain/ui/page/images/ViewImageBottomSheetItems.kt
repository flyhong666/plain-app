...
internal fun ViewImageActionButtons(
    ...
    if (showAddToHomeDialog) {
        AddToHomeDialog(
            defaultLabel = remember(m.path) { File(m.path).nameWithoutExtension },
            onAddToHome = { label -> MediaShortcutHelper.addToDesktop(context, m.path, label, AppResources.mipmap("ic_launcher")) },
            onDismiss = {
