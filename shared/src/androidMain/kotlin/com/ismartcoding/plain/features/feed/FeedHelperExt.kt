...
suspend fun FeedHelper.importAsync(reader: Reader) = withIO {
    val feedList = mutableListOf<DFeed>()
    val opml = OpmlParser().parse(reader.readText())
...
suspend fun FeedHelper.exportAsync(writer: Writer) = withIO {
    val feeds = getAll()
    val result =
        OpmlWriter().write(
            Opml(
