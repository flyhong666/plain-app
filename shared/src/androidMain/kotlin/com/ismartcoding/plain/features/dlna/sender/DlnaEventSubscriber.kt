...
object DlnaEventSubscriber {
    ...
    suspend fun subscribeEvent(device: DlnaDevice, callbackUrl: String): String {
        ...
        return try {
            val response = withIO {
                HttpClient(OkHttp).subscribe(device.getBaseUrl() + "/" + service.eventSubURL.trimStart('/')) {
                    headers {
                        set("NT", "upnp:event")
    ...
    suspend fun unsubscribeEvent(device: DlnaDevice, sid: String): String {
        ...
        return try {
            val response = withIO {
                HttpClient(OkHttp).unsubscribe(device.getBaseUrl() + "/" + service.eventSubURL.trimStart('/')) {
                    headers { set("SID", sid) }
                }
            }
