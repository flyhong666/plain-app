...
fun HomeWebMainSection(
    ...
    val collapsed = remember {
        appDataStore.dataFlow.map { HomeSectionCollapsedPreference.get(it, AppFeatureType.WEB_PORTAL) }
    ...
    Column {
        ...
        if (webState != WebState.OFF && (webState != WebState.ON || !collapsed)) {
            VerticalSpace(12.dp)
            HomeWebAddressSection(navController, webState == WebState.ERROR) { onExit ->
                StayOnlineModeOverlay(onExit = onExit)
            }
        }
    }
}
